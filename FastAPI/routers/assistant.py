...
async def completion(
    ...
    encoded_images: Optional[List[str]] = Form(None),
    model: AIModel = Form(AIModel.GPT_4O),
    generate_audio: bool = Form(False)
):
    return await assistant_service.completion(
        db, user, text, audio, images, encoded_images, model, generate_audio
    )
